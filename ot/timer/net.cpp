...
void Rct::_update_ldelay(RctNode * parent, RctNode * from) {
  int i, j;
  std::vector<std::vector<float>> to_ldelay_acc;

  for (auto e: from->_fanout) {
    if (auto & to = e->_to; & to != parent) {
      _update_ldelay(from, & to);
      std::vector <float> to_ldelay_RctNode;
      FOR_EACH_EL_RF(el, rf) {
        to_ldelay_RctNode.push_back(to._ldelay[el][rf]);
      }
      to_ldelay_acc.push_back(to_ldelay_RctNode);
    }
  }

  int num_nodes = to_ldelay_acc.size();
  if (num_nodes > 1) {
    /***************** GPU EXECUTION *****************/
    float *to_float_star = new float[num_nodes * 4];
    float from_ldelay[4], from_cap[4], from_delay[4];

    for (i = 0; i < num_nodes; i++) {
      for (j = 0; j < 4; j++) {
        to_float_star[i * 4 + j] = to_ldelay_acc[i][j];
      }
    }

    FOR_EACH_EL_RF(el, rf) {
      from_cap[el*2+rf] = from->cap(el, rf);
      from_delay[el*2+rf] = from->_delay[el][rf];
    }

    #pragma acc data copyin(to_float_star[:num_nodes*4], from_cap[:4], from_delay[:4])
    #pragma acc data copyout(from_ldelay[:4])
    #pragma acc parallel loop
    for (i = 0; i < 4; i++) {
      float acc = 0.0;
      for (j = 0; j < num_nodes; j++) {
        acc += to_float_star[i + j * 4];
      }
      from_ldelay[i] = acc + from_cap[i] * from_delay[i];
    }

    delete[] to_float_star;

    FOR_EACH_EL_RF(el, rf) {
      from->_ldelay[el][rf] = from_ldelay[el * 2 + rf];
    }
    ...
    for (i = 0; i < num_nodes; i++) {
      for (j = 0; j < 4; j++) {
        from->_ldelay[i][j] += to_ldelay_acc[i][j];
      }
    }
    FOR_EACH_EL_RF(el, rf) {
